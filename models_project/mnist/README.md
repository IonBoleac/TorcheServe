 ...
 * Step - 4: Create a torch model archive using the torch-model-archiver utility to archive the above files.
    ...
    ```bash
    torch-model-archiver --model-name mnist --version 1.0 --model-file ./models_project/mnist/mnist.py --serialized-file ./models_project/mnist/mnist_cnn.pt --handler ./models_project/mnist/mnist_handler.py --extra-files ./models_project/mnist/mnist_ts.json -f --requirements-file ./models_project/mnist/mnist_requirements.txt
    ```
